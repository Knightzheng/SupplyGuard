...
| 当前版本 | v0.1.0.dev0 |
| 当前工作重点 | 设计统一组件模型并实现首个 package-lock 解析器 |
| 已完成 | 治理文档、Git 仓库、独立虚拟环境、最小 CLI、离线检查脚本、3 项 CLI 测试 |
| 进行中 | 无 |
| 下一任务 | 定义 Component、DependencyEdge、SourceEvidence，并以测试驱动实现 package-lock v2/v3 解析 |
| 当前阻塞 | 无 |
| 额外金钱成本 | 0 元 |
| 最后更新时间 | 2026-06-21 16:30，Asia/Shanghai |
...
1. 检查项目文件树和 Git 状态。
2. 为 M1 定义不可变的 `Component`、`DependencyEdge` 和 `SourceEvidence` 领域模型及稳定 ID 规则。
3. 先添加模型不变量、顺序确定性和非法输入测试，再实现模型。
4. 以最小 package-lock v2/v3 夹具开始解析器，不执行待扫描项目代码。
5. 运行 `scripts\check.ps1`，确认现有 CLI 回归测试和新增测试均通过。
6. 更新本文件的当前快照并追加推进记录。
